...
class Input
{
	...
	{
		if( isset($_ENV[$str]) )		return $_ENV[$str];
		if( isset($_SERVER[$str]) )		return $_SERVER[$str];
		if( isset($_FILES[$str]) )		return $_FILES[$str];
	...
	public static function get($str, $default='', $clean=Input::CLEAN_HTML)
	{
		if( isset($_GET[$str]) )
		{
			$val = Input::clean_data($_GET[$str], $clean);
	...
	public function param($str, $default='', $clean=Input::CLEAN_HTML)
	{
		if( isset($_POST[$str]) )
		{
			$val = Input::clean_data($_POST[$str], $clean);
	...
	function clean_keys($str)
	{
		if( preg_match("/^[a-z0-9:_\/-]+$/i", $str) )	return $str;
		else											return '';
	}
